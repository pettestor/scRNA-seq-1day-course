...
install.packages(c("Seurat", "dplyr", "ggplot2", "robustbase", "tibble","viridis","SingleR","scRNAseq","patchwork","org.Hs.eg.db",
                   "scater", "remotes", "harmony","clusterProfiler"))
...
ourselves with the method and gain insights into its application. One
common approach to identify doublets is using the **DoubletFinder**
...
da_diff@meta.data %>% ggplot(aes(x=nFeature_RNA, y=nCount_RNA, color=doublet_classification)) + geom_point() + scale_color_manual(values=c("black", "red"))+facet_wrap(~orig.ident+doublet_classification)
...
The total number of UMIs observed for a cell are attributable to 1) cell
size (higher cell size =\> more RNA content =\> more expected UMIs) and
2) sequencing depth, which will have a large random component. Let’s
look at the expression of a ubiquoulsy expressed gene that has been
described as a housekeeping gene in snRNA-seq data (*MALAT1*- a long
noncoding RNA):

```{r malat1}
total_umis <- colSums(x = GetAssayData(da_diff.filter,layer = "counts"))
gene <- "MALAT1"
gene_umi <- GetAssayData(da_diff.filter,layer = "counts")["MALAT1",]
...
The gene *MALAT1* exhibits an almost linear relationship with a cell's
sequencing depth. In this case, the "heterogeneity across cells" in
*Malat1* expression levels is primarily due to differences in sequencing
depth rather than biological variability, making it less biologically
interesting.
...
depth of each cell. To minimize the influence of outliers, the scaled
values are log-transformed after adding a pseudocount of 1. In Seurat,
this normalization can be performed using the `NormalizeData` function:

```{r normalization}
# Normalize data
da_diff.filter <- NormalizeData(da_diff.filter)

gene_umi_normalized <- GetAssayData(da_diff.filter,layer = "data")["MALAT1",]

ggplot(data = data.frame("total_cell_umi" = total_umis, "gene_umi_norm" = gene_umi_normalized), aes(total_cell_umi, gene_umi_norm)) +
  geom_point() +
  theme_bw(base_size = 14) +
  xlab("Total sequencing depth") +
...
da_diff.filter <- FindClusters(da_diff.filter, resolution = 0.2)
...
DimPlot(da_integrated, reduction = "umap_harmony", group.by = "seurat_clusters") +
...
relevant genes and calculate a module score for each cluster. This can
help us identify the most relevant genes for each cluster and
potentially identify new markers.
...
featureplot.gene_modules <- FeaturePlot(
    da_integrated,
...
violinplot.gene_modules <- VlnPlot(
    da_integrated,
    features = paste0(names(gene_modules),"1"),
    pt.size = 0)

violinplot.gene_modules / featureplot.gene_modules
...
Here we will use the Seurat funciton *FindAllMarkers* to identify marker
genes for each cluster. We will then visualize the top 10 marker genes
for each cluster using a heatmap.
...
In single-cell RNA sequencing (scRNA-seq) studies, identifying cell
types within a heterogeneous population is a key challenge. One way to
classify cells is by using reference-based classification methods, which
rely on prior knowledge of gene expression profiles for known cell
types. SingleR is a powerful tool for this task, which performs
reference-based classification of single-cell data by comparing the gene
expression profiles of the cells in your dataset to a reference dataset
...
classify cells from a different dataset (da_integrated). The
classification results are then added to the Seurat object as metadata,
allowing us to visualize and analyze the cell type predictions.
...
DimPlot(da_integrated, group.by = "filtered_label_transfer", reduction = "umap",label = T)+
  FeaturePlot(da_integrated, features = c("prediction.score.max"),order=T,min.cutoff = "q9")
```
...
genes identified in **Section 6.2**. The **clusterProfiler** package
