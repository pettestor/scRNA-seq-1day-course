...
We will use the four midbrain organoid samples sequenced using 10X scRNA-seq at two timepoints. The data set comes from this [publication](https://pubmed.ncbi.nlm.nih.gov/33445654/).

```{r load-data}
# List all directories within the "data/" directory, excluding the top-level "data/" itself.
dirs <- list.dirs("../scRNAseq-course/data/organoids/")[-1]
...
da_diff$DoubletComparison <- ifelse(
  da_diff$DF.classifications_0.25_0.1_508 == "Doublet" & da_diff$scdblfinder.predicted_doublet == "doublet", "Both",
  ifelse(da_diff$scdblfinder.predicted_doublet == "doublet", "scDblFinder Only",
  ifelse(da_diff$DF.classifications_0.25_0.1_508 == "Doublet", "DoubletFinder Only", "Neither"))
)


# Scatter plot comparing doublet scores
ggplot(da_diff@meta.data, aes(x = scdblfinder.doublet_scores, y = pANN_0.25_0.1_508)) +
...
1.  **Examine the expression of canonical cell type and cell state markers** within the clusters. This method relies on established knowledge of well-characterized markers in the field.

2.  **Identify signature or marker genes** for each cell cluster. Once these genes are determined, literature searches, enrichment analyses, or experimental validations can be conducted to further refine annotations.

3.  **Compare the gene expression profiles of the clusters** with existing reference datasets. This can provide additional context for identifying the nature of the clusters.
...
In some scRNA-seq experiments, cells may be derived from multiple species, such as human-mouse xenografts.
We will start by creating the seurat object.

```{r create_multi_species_seu}
dirs <- list.dirs("~/Dropbox/Parmar/jb001/filtered_bc_matrix/")[-1]

# Rename each directory by removing the "data/FGF8plus_" 
# prefix from the directory names.
# This step makes sure that the "orig.ident" variable in your Seurat object will be informaticve.
#names(dirs) <- gsub("/Users/petter/Dropbox/Parmar/jb001/filtered_bc_matrix//","",dirs)
#names(dirs) <- gsub("_30May2024","",names(dirs))
#names(dirs) <- gsub("_Graft29weekspost","",names(dirs))
...
# The 'Read10X' function reads the gene expression count data for each sample into one large matrix
da_diff.data <- Read10X(data.dir = dirs)
rownames(da_diff.data)<-gsub("grch38_","",rownames(da_diff.data))
...
```{r spec}
# Load species classification CSV files from a specified directory
gem_classification.files <- list.files(
  "~/Dropbox/Parmar/jb001/filtered_bc_matrix/", # Directory containing the files
  pattern = "*.csv",                           # Filter to include only CSV files
  full.names = TRUE                            # Return full file paths
)

# Read and combine the data from all CSV files
gem_classification <- gem_classification.files %>%
  lapply(function(file) {                         # Apply the following function to each file:
    df <- read.csv(file)                          # Read the current file into a data frame
    df$filename <- gsub(                          # Add a new column 'filename'
      "_", "-",                                   # Replace underscores with dashes
      gsub(".gem_classification.csv", "",         # Remove the ".gem_classification.csv" suffix
           basename(file))                        # Extract the base name of the file
    )
    return(df)                                    # Return the modified data frame
  }) %>%
  bind_rows()                                     # Combine all data frames into a single data frame

# Set unique row names for the combined data frame using filename and barcode
rownames(gem_classification) <- paste0(
  gem_classification$filename,                   # Include the filename
  "_",                                           # Separator
  gem_classification$barcode                     # Include the barcode
)

# Add species classification metadata to the Seurat object
da_diff <- AddMetaData(
  da_diff,                                       # Existing Seurat object
  gem_classification[, c("grch38", "rnor6", "call")] # Add specific columns: grch38, rnor6, call
)

# Simplify and standardize the 'orig.ident' metadata for the Seurat object
da_diff$orig.ident <- gsub(
  "-30May2024", "",                              # Remove the date suffix
  gsub("-Graft29weekspost-TX", "",               # Remove the "Graft29weekspost-TX" suffix
       da_diff$orig.ident)                       # Target the 'orig.ident' column
)

# Set cell identities to the modified 'orig.ident' values
Idents(da_diff) <- da_diff$orig.ident
