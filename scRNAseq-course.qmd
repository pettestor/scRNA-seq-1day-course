...
\`\`\`{r spec} \# Load species classification CSV files from a specified
directory gem_classification.files \<- list.files(
"\~/Dropbox/Parmar/jb001/filtered_bc_matrix/", \# Directory containing
the files pattern = "\*.csv", \# Filter to include only CSV files
full.names = TRUE \# Return full file paths )

# Read and combine the data from all CSV files

gem_classification \<- gem_classification.files %\>%
lapply(function(file) { \# Apply the following function to each file: df
\<- read.csv(file) \# Read the current file into a data frame
df\$filename \<- gsub( \# Add a new column 'filename' "\_", "-", \#
Replace underscores with dashes gsub(".gem_classification.csv", "", \#
Remove the ".gem_classification.csv" suffix basename(file)) \# Extract
the base name of the file ) return(df) \# Return the modified data frame
...
da_diff \<- AddMetaData( da_diff, \# Existing Seurat object
gem_classification\[, c("grch38", "rnor6", "call")\] \# Add specific
columns: grch38, rnor6, call )
...
In this tutorial, we've covered the fundamentals of scRNA-seq data
analysis using Seurat, from loading and preprocessing data to clustering
and marker identification. For further learning, explore the [Seurat
documentation](https://satijalab.org/seurat/) and try using your own
dataset.
